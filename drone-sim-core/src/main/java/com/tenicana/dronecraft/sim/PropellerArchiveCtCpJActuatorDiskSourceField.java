...
public record PropellerArchiveCtCpJActuatorDiskSourceField(
	...
	public record SourceFieldSample(
			...
			Vec3 freestreamVelocityWorldMetersPerSecond
	) {
		public SourceFieldSample {
			...
			freestreamVelocityWorldMetersPerSecond =
					finiteVecOrZero(freestreamVelocityWorldMetersPerSecond);
		}
	...
	public record VoxelCellSample(
		...
		public Vec3 wakeSkewLateralVelocityWorldMetersPerSecond() {
			return targetWakeVelocityWorldMetersPerSecond
					...
					.subtract(wakeSwirlVelocityWorldMetersPerSecond);
		}
	...
	public SourceFieldSample sampleAt(Vec3 samplePointWorldMeters) {
		...
		Vec3 freestreamVelocity = Vec3.ZERO;
		...
		Vec3 fallbackFreestreamVelocity = Vec3.ZERO;
		...
		for (PropellerArchiveCtCpJRotorForceModel.RotorActuatorDiskSourceTermSample sourceTerm : sourceTerms) {
			...
			fallbackFreestreamVelocity = fallbackFreestreamVelocity.add(sourceFreestreamVelocity);
			if (sourceMassFlux > EPSILON) {
				...
				freestreamVelocity = freestreamVelocity.add(sourceFreestreamVelocity.multiply(sourceMassFlux));
		...
		if (wakeVelocityMassFluxWeight > EPSILON) {
			...
			freestreamVelocity = freestreamVelocity.multiply(inverseWakeVelocityWeight);
			...
			freestreamVelocity = fallbackFreestreamVelocity.multiply(inverseContributingSources);
		}
	...
	private VoxelCellSample sampleVoxelCell(
			...
			for (int sz = 0; sz < subcellSamplesPerAxis; sz++) {
				for (int sx = 0; sx < subcellSamplesPerAxis; sx++) {
					...
					if (sampleMassFlux > EPSILON) {
						...
						freestreamVelocity = freestreamVelocity.add(
								sample.freestreamVelocityWorldMetersPerSecond().multiply(sampleMassFlux));
						...
						fallbackFreestreamVelocity =
								fallbackFreestreamVelocity.add(sample.freestreamVelocityWorldMetersPerSecond());
					}
				}
			}
		}
		...
		if (wakeVelocityMassFluxWeight > EPSILON) {
			...
			freestreamVelocity = fallbackFreestreamVelocity.multiply(inverseFallbackActiveSamples);
		}
	...
	private VoxelCellSample sampleConservativeVoxelCell(
		...
		for (SourceCoverage coverage : coverages) {
			...
			if (sourceMassFluxWeight > EPSILON) {
				...
				freestreamVelocity = freestreamVelocity.add(sourceFreestreamVelocity.multiply(sourceMassFluxWeight));
				...
				fallbackFreestreamVelocity =
						fallbackFreestreamVelocity.add(sourceFreestreamVelocity.multiply(velocitySampleWeight));
		...
		if (wakeVelocityMassFluxWeight > EPSILON) {
			...
			freestreamVelocity = fallbackFreestreamVelocity.multiply(inverseFallbackVelocityWeight);
		}
		return new VoxelCellSample(
				xIndex,
				yIndex,
			...
			if (sourceTerm != null && sourceTerm.applied()) {
				sum = sum.add(sourceTerm
						.equivalentWakeAngularMomentumTorqueDensityWorldNewtonMetersPerCubicMeter(
								sourceThicknessMeters)
						.multiply(sourceTerm.wakeSupportVolumeCubicMeters(sourceThicknessMeters)));
			}
		}
		return sum;
	}
	...
	private List<SourceCoverage> sourceCoverages(VoxelGridSpec gridSpec, int subcellSamplesPerAxis) {
		...
		for (PropellerArchiveCtCpJRotorForceModel.RotorActuatorDiskSourceTermSample sourceTerm : sourceTerms) {
			...
			double targetWakeTorqueVolume = sourceTerm.wakeSupportVolumeCubicMeters(sourceThicknessMeters);
			double densityScale = sampledVolume > EPSILON && targetVolume > EPSILON
					? targetVolume / sampledVolume
					: 0.0;
			double wakeTorqueDensityScale = sampledWakeTorqueVolume > EPSILON && targetWakeTorqueVolume > EPSILON
					? targetWakeTorqueVolume / sampledWakeTorqueVolume
					: 0.0;
	...
	private double wakeAngularMomentumWeightedSampledVolumeCubicMeters(
		...
		double sampledWakeSupportVolume = 0.0;
		for (int y = 0; y < gridSpec.cellCountY(); y++) {
			for (int z = 0; z < gridSpec.cellCountZ(); z++) {
				for (int x = 0; x < gridSpec.cellCountX(); x++) {
					Vec3 cellCenter = gridSpec.cellCenterWorldMeters(x, y, z);
							...
							for (int sx = 0; sx < subcellSamplesPerAxis; sx++) {
								...
								if (containsActuatorDiskVolume(sourceTerm, subcellPoint)) {
									if (sourceTerm.containsWakeSwirlSupport(subcellPoint)) {
										sampledWakeSupportVolume += sampledSubcellVolume;
									}
		...
		if (weightedVolume > EPSILON) {
			return weightedVolume;
		}
		return sampledWakeSupportVolume > EPSILON ? sampledWakeSupportVolume : 0.0;
	}
	...
	private SourceCellCoverage sourceCellCoverage(
		...
		return new SourceCellCoverage(
				...
				wakeSupportSubsamples > 0 && averageWakeTorqueWeight <= EPSILON
						? 1.0
