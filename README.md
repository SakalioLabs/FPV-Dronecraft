...
Rotor blade count is now part of each `RotorSpec`; 5-inch FPV-style `racingQuad` and `cinewhoop` presets use three-blade blade-pass ripple and gyro notch frequencies while larger lift presets keep the two-blade default. The live `/fpvdrone tune rotor_blade_count` command persists with the drone entity, `/fpvdrone tune rotor_pitch_to_diameter` lets prop geometry be entered in the same pitch/diameter style as 5x4.3 or 5x4.5 real props, and blackbox/offline CSV output includes `tune_rotor_blade_count`, `tune_rotor_pitch_to_diameter`, and `tune_rotor_pitch_angle_70r_deg` beside blade pitch for log-to-prop-spectrum validation.
...
Battery sag, bus ripple, regenerative voltage spikes, and motor current ripple now feed an `imu_supply_noise` telemetry channel. It is applied to gyro, accelerometer, and barometer noise, appears live as `pwr`/`P`, and blackbox/offline summaries report `imuP`/`max_imu_power_noise` so electrical-noise-driven sensor trouble can be separated from mechanical vibration. The bus ripple source is also exposed live as `ripple`/`Rp` and raises `bus-ripple` before it is hidden behind the aggregate IMU power-noise channel.
The runtime LiPo model also exposes SOC-, temperature-, cycle-aging-, and high-current-polarization-adjusted pack resistance as `battery_effective_resistance_ohm` in blackbox/offline CSV output. Its SOC curve keeps ordinary `R0` rise close to open LiPo ECM data while reserving the steepest penalty for the near-empty knee, its cold-temperature ESR rise is shaped against RC LiPo 71F-to-42F resistance-ratio measurements, and the separate `battery_resistance_aging_scale`, `battery_polarization_resistance_scale`, and `battery_equivalent_cycles` columns track slow cycle growth plus reversible punch-out polarization without changing each preset's high-C baseline ESR. `/fpvdrone status` prints effective IR as `ir` in mOhm, blackbox summaries report peak `ir`, and offline reports include `max_ir`, so voltage sag can be tuned against measured pack internal resistance instead of inferred only after the fact.
Airframe drag is split between true low-speed linear damping `F=-cv` and body-axis quadratic coefficients `F_i=-c_i v_i |v_i|`; the default racing quad is calibrated near the IMAV 5-inch drag scale at 10 m/s while separated-flow, rotor-wash, and near-ground cushion terms remain separate telemetry channels.
...
Rotor side-flow blockage near walls now aggregates rotor-disk edge coverage instead of using only the nearest ray, so single-wall, corner, and tunnel-like Minecraft geometry produce different obstruction intensity and wall-attraction direction before thrust loss, cooling loss, vibration, and wall-effect force are applied.
Vortex-ring-state mean thrust loss is calibrated against the small-prop reference band around `1.2..1.3 vi`, so a fully loaded vertical descent can lose roughly one third of clean thrust before the separate low-frequency VRS buffet adds conservative but visible Shetty/Selig-style unsteady thrust and lateral-force texture.
...
Blackbox summaries include `coax`, the peak balanced upper/lower coaxial load-bias allocation for X8 tuning.
...
- geometry-derived multirotor mixer using each rotor's force arm and actual thrust axis relative to the configured center of mass, plus configurable rotor outward cant, benchmark-spacing-window coaxial upper/lower load bias with a New Dexterity z/D=0.72 command-map allocation prior for complete X8 stacks, and airframe center-of-pressure moment arms for drag/lift torque, currently exposed through quad, cinewhoop, heavy-lift quad, six-rotor hex-lift, eight-rotor octo-lift, and compact coaxial-X8 presets, with per-rotor thrust and yaw reaction torque.
