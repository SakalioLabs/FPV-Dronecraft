...
- 这个修复仍然保留穿越机 rate mode 的核心语义：`450°` 这种等价 90° bank / knife-edge 的姿态不会被吞掉，仍然会产生强侧向推力；只有“已经超过一整圈、松杆、残差仍在最大可玩倾角附近”的情况被视作完成动作。
- ACRO 空阻也从 yaw 局部轴改成 pitch/roll 参与的机体系 right/up/forward 三轴阻力，再投回当前局部坐标；翻滚后残留侧滑不会再被一个不随机体姿态旋转的平面阻力模型处理。
- 新增回归测试覆盖 `428° roll`、`425° pitch loop` 松杆后的目标速度归零，以及 `450°` 真实侧倾仍保留强水平推力；同时加入机体系速度投影往返测试，防止跨过 360° 后坐标轴失真。
