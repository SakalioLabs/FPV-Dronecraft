...
Rotor blade count is now part of each `RotorSpec`; 5-inch FPV-style `racingQuad` and `cinewhoop` presets use three-blade blade-pass ripple and gyro notch frequencies while larger lift presets keep the two-blade default. Blackbox/offline CSV output includes `tune_rotor_blade_count` beside blade pitch for log-to-prop-spectrum validation.
