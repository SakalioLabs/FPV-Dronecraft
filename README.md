...
Low-throttle reverse-axial-flow windmilling is logged as `rotor_windmilling` plus per-rotor columns through `rotor_7_windmilling`; live status/HUD expose it as `wmill`/`WM`, blackbox summaries print `wmill`, and offline reports include `max_windmill`.
