...
The runtime LiPo model also exposes SOC-, temperature-, and cycle-aging-adjusted pack resistance as `battery_effective_resistance_ohm` in blackbox/offline CSV output. Its SOC curve keeps ordinary `R0` rise close to open LiPo ECM data while reserving the steepest penalty for the near-empty knee; the separate `battery_resistance_aging_scale` and `battery_equivalent_cycles` columns track the Mendeley-style slow resistance growth without changing each preset's high-C baseline ESR. `/fpvdrone status` prints effective IR as `ir` in mOhm, blackbox summaries report peak `ir`, and offline reports include `max_ir`, so voltage sag can be tuned against measured pack internal resistance instead of inferred only after the fact.
