...
- 新增并跑通无头服务端 `playable` 可玩飞行层自测；诊断脚本现在使用玩家默认的 `ANGLE` 首飞模式：12 秒脚本飞行、最大爬升约 `1.85 m`、最大速度约 `4.24 m/s`、最终速度 `0.00 m/s`、最终水平距离约 `0.06 m`、平均电机遥测峰值约 `11.94k RPM`、249 个样本；黑盒 CSV 会明确写入 `flight_model=playable` 和 `flight_mode=angle`，并记录可玩层的 `control_*`、ESC 输出、RPM telemetry 和 per-rotor 推力。
...
- 黑盒 CSV、`/fpvdrone blackbox summary` 和无头服务端自测现在额外记录 `playable` 可玩层真实可视 pitch/roll、偏航速率和首尾 yaw 漂移；最新默认 `ANGLE` playable 自测最大可视 pitch 约 `2.00 deg`、roll 约 `2.18 deg`、yaw rate 约 `4.98 dps`、最终 yaw 漂移 `0.00 deg`，后续调参可以直接看玩家画面姿态是否过敏或自转。
- 服务端 `playable` 输入滤波现在在摇杆回中和反向过零时会快速归零残余命令，减少松杆后几帧仍继续倾斜/偏航的拖尾；这部分有独立单元测试覆盖，最新默认 `ANGLE` playable 无头自测仍保持最终速度 `0.00 m/s`、最终水平距离约 `0.06 m`、最终 yaw 漂移 `0.00 deg`。
