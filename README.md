...
Rotor blade count is now part of each `RotorSpec`; 5-inch FPV-style `racingQuad` and `cinewhoop` presets use three-blade blade-pass ripple and gyro notch frequencies while larger lift presets keep the two-blade default. The live `/fpvdrone tune rotor_blade_count` command persists with the drone entity, and blackbox/offline CSV output includes `tune_rotor_blade_count` beside blade pitch for log-to-prop-spectrum validation.
...
The runtime LiPo model also exposes SOC-, temperature-, and cycle-aging-adjusted pack resistance as `battery_effective_resistance_ohm` in blackbox/offline CSV output. Its SOC curve keeps ordinary `R0` rise close to open LiPo ECM data while reserving the steepest penalty for the near-empty knee, its cold-temperature ESR rise is shaped against RC LiPo 71F-to-42F resistance-ratio measurements, and the separate `battery_resistance_aging_scale` and `battery_equivalent_cycles` columns track the Mendeley-style slow resistance growth without changing each preset's high-C baseline ESR. `/fpvdrone status` prints effective IR as `ir` in mOhm, blackbox summaries report peak `ir`, and offline reports include `max_ir`, so voltage sag can be tuned against measured pack internal resistance instead of inferred only after the fact.
