...
Coaxial X8 load sharing is logged as `rotor_coaxial_load_bias` plus signed per-rotor columns through `rotor_7_coaxial_load_bias`, where upper rotors are positive and lower rotors are negative; status/HUD expose it as `coax`/`CX`, blackbox summaries print `coax`, offline reports print `max_coax_bias`, and `coax-load-bias` warns when upper/lower thrust allocation becomes a live X8 tuning risk.
