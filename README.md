...
Battery sag, bus ripple, regenerative voltage spikes, and motor current ripple now feed an `imu_supply_noise` telemetry channel. It is applied to gyro, accelerometer, and barometer noise, appears live as `pwr`/`P`, and blackbox/offline summaries report `imuP`/`max_imu_power_noise` so electrical-noise-driven sensor trouble can be separated from mechanical vibration.
