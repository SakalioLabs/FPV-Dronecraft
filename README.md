...
The HUD torque line includes active-braking reaction torque as `BT`, gyroscopic/precession torque as `GT`, and flapping force-arm torque as `FT`, next to blade-dissymmetry and wake-swirl hub torque.
...
`rotor_inertia` still drives spin-up reaction torque and body-rate gyroscopic/precession coupling, and the gyroscopic component is logged separately as `gyroT`/`GT` plus `rotor_gyroscopic_*_torque_nm` for tuning.
