...
The runtime LiPo model also exposes SOC- and temperature-adjusted pack resistance as `battery_effective_resistance_ohm` in blackbox/offline CSV output. `/fpvdrone status` prints it as `ir` in mOhm, blackbox summaries report peak `ir`, and offline reports include `max_ir`, so voltage sag can be tuned against measured pack internal resistance instead of inferred only after the fact.
