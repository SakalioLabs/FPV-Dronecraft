...
public final class Aerodynamics4McL2PoweredHoverSourceMap {
	public static final String SOURCE_ID = "A4MC-L2-Powered-Hover-Source-Map-Packet";
	public static final String CAVEAT =
			"Hover source-map packet expands actuator-disk hover pressure jumps into per-rotor force and moment source targets; use only as a future A4MC powered source-term request target.";
	...
	private Aerodynamics4McL2PoweredHoverSourceMap() {
	}

	public record PoweredHoverSourceMap(
			String presetName,
			String spinState,
			int rotorCount,
			int sourceTermCount,
		...
		public PoweredHoverSourceMap {
			sourceTerms = List.copyOf(sourceTerms);
		}
	}

	public record PoweredHoverSourceMapExtrema(
	...
	public record PoweredHoverSourceMapAudit(
			String sourceId,
			String caveat,
			...
			List<PoweredHoverSourceMap> sourceMaps,
			PoweredHoverSourceMapExtrema extrema
	) {
		public PoweredHoverSourceMapAudit {
			sourceMaps = List.copyOf(sourceMaps);
		}
	}

	public static PoweredHoverSourceMapAudit audit() {
		List<PoweredHoverSourceMap> sourceMaps = List.of(
		...
		return new PoweredHoverSourceMapAudit(
				SOURCE_ID,
				CAVEAT,
	...
	public static PoweredHoverSourceMap sourceMap(String presetName, DroneConfig config, Vec3 inletVelocity, int steps) {
		...
		Aerodynamics4McL2ActuatorDiskCalibration.ActuatorDiskLoad load =
				Aerodynamics4McL2ActuatorDiskCalibration.loads(presetName, config, inletVelocity, steps).stream()
						.filter(candidate -> "hover".equals(candidate.spinState()))
						.findFirst()
						.orElseThrow();
		double inletSpeed = inletVelocity == null ? 0.0 : inletVelocity.length();
		return new PoweredHoverSourceMap(
				map.presetName(),
				map.spinState(),
				map.rotorCount(),
	...
	private static PoweredHoverSourceMapExtrema extrema(List<PoweredHoverSourceMap> sourceMaps) {
		...
		for (PoweredHoverSourceMap sourceMap : sourceMaps) {
		...
		return new PoweredHoverSourceMapExtrema(
				maxMeanPressureJump,
				maxNetForce,
				maxNetMoment,
				maxPower,
