...
public final class DroneStatusFormatter {
	...
	public static String format(Telemetry telemetry) {
		...
		return String.format(
				Locale.ROOT,
				"Drone status | mode %s armed %s link raw %s fc %s failsafe %s %.2fs rc %.3f/%.3fs err %.4f | input T %.2f P %.2f R %.2f Y %.2f | speed %.2fm/s contact %.2f/%.2f/%.2fm/s %.0fd/s air %.2fm/s AoA %.1f slip %.1f | forces lift %.1fN sep %.2f flap %.1fdeg cushion %.1fN glev %.3fNm wash %.1fN wall %.1fN | baro %.1fm %.1fm/s %.1fhPa err %.2fm | battery %.2fV %.0f%% sag %.2fV ir %.0fmOhm irx %.2f/%.2f/%.2f sag20 %.0fA margin %.2f spike %.2fV ripple %.3fV %.1fA regen %.1fA limit %.2f current-limit %.2f | imu clip G %.2f A %.2f pwr %.2f dterm %.0fHz | health frame %.0f%% rotor %.0f%% motor %.1fC %.2f head %.2f mR %.2f esc %.1fC %.2f cool %.2f sig %.3f/%.3fs err %.4f desync %.2f load %.2f hforce %.2fN scrape %.2f prop strikes %d last %s | aero propwash %.2f VRS %.2f vrsbuf %.0f%% vrsF %.2fN ETL %.2f ind %.2fm/s iloss %.0f%% adv %.2f J %.2f pthr %.2f ppwr %.2f rev %.2f tipmach %.2f machloss %.0f%% lowre %.2f blade %.0fdeg bstall %.2f bpass %.3f bdiss %.3fNm skew %.2f rwake %.2f wloss %.0f%% coax %.3f wetloss %.0f%% ice %.2f iceloss %.0f%% icepwr %.2f swirl %.2fm/s wmill %.2f swirlT %.3fNm brakeT %.3fNm accelT %.3fNm gyroT %.3fNm flapT %.3fNm wake %.2f ceil %.2f asym %.2f blk %.2f water %.2f rain %.2f temp %.1fC stall %.2f vib %.2f coning %.2f mixer %.2f wind %.1fm/s airmass %.1fm/s gust %.2fm/s shear %.2fm/s2 turb %.2f obs %.2f ground %.2f | blackbox %d/%d | %s | warnings %s",
				...
				telemetry.groundEffectLevelingTorqueNewtonMeters(),
				...
				telemetry.batteryStateOfChargeResistanceScale(),
				telemetry.batteryTemperatureResistanceScale(),
				telemetry.batteryPolarizationResistanceScale(),
				...
				telemetry.rotorIcingSeverity(),
				(1.0 - telemetry.rotorIcingThrustScale()) * 100.0,
				telemetry.rotorIcingPowerScale(),
	...
	static String warnings(Telemetry telemetry) {
		...
		if (telemetry.groundEffectLevelingTorqueNewtonMeters() > 0.045) {
			warnings.add("ground-leveling-torque");
		}
		...
		if (telemetry.rotorIcingSeverity() > 0.08 || telemetry.rotorIcingThrustScale() < 0.985 || telemetry.rotorIcingPowerScale() > 1.04) {
			warnings.add("rotor-icing");
		}
	...
	public record Telemetry(
			...
			double groundEffectLevelingTorqueNewtonMeters,
