...
- `docs/data/a4mc_source_quality_response_packet.csv` adds a `4574`-row A4MC source-quality response matrix mirrored as `a4mc_source_quality_packet_*`. It varies trust, confidence, freshness, shelter, shear, and source turbulence, then records the core-adopted source turbulence, natural turbulence, source gust, terrain-shear, local-voxel ventilation, and local-obstacle residual responses.
...
- The A4MC source-quality packet keeps the synthetic wall-skim source gate explicit: the reference `confidence=0.86`, fresh, `shelter=0.74`, `shear=0.58/block` case adopts source quality `0.86`, gives a source-gust Y peak proxy of `0.409 m/s`, a terrain-vector peak proxy of `0.192 m/s`, and local-voxel ventilation efficiency `0.873`. A stale `160 tick` source drops quality and source-gust forcing to zero.
