...
- `docs/data/a4mc_local_voxel_coupling_packet.csv` adds a `3528`-row A4MC local-voxel bridge, body-response, and thermal ventilation audit mirrored as `a4mc_local_voxel_packet_*`. It isolates the fabric-side preprocessing that turns trusted local L2 A4MC shelter and pressure samples into source-quality-gated duplicate-obstacle residuals, precipitation exposure attenuation, rotor residual fallback behavior, pressure-gradient disk-wind equivalents, high-contrast pressure saturation ordering, and per-sample quality-blended shelter-gradient side obstruction before the core disk-gradient response, then adds `54` core pressure-center scenarios plus `6` motor/ESC/battery ventilation scenarios for adopted asymmetric local-voxel rotor signals. Runtime blackbox and offline CSV output now expose aggregate and per-rotor `local_voxel_obstacle_residual`, precipitation wetness, `rotor_a4mc_ventilation_efficiency`, per-rotor A4MC ventilation efficiency, and `a4mc_pack_ventilation_efficiency` columns, while the blackbox summary reports the minimum rotor/pack A4MC ventilation efficiency as `a4mcvent` and the offline CLI `Summary:` line reports the same minima as `min_a4mc_vent=rotor/pack`, so trace analysis can line up live wall/tunnel attenuation, wet-prop shelter relief, pressure-center moments, and local shelter cooling loss with this packet directly.
