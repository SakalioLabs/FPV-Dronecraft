...
- `docs/data/a4mc_source_quality_response_packet.csv` adds a `17649`-row A4MC source-quality response matrix mirrored as `a4mc_source_quality_packet_*`. It varies trust, confidence, freshness, shelter, shear, source turbulence, dedicated ABL stability/mixing, signed updraft/downwash adoption, and pressure-anomaly response, then records the core-adopted source turbulence, natural turbulence, ABL Dryden intensity/time-scale, source gust, de-overlapped adopted updraft target/settling response, terrain-shear, pressure-density/barometer/static-port response, local-voxel ventilation, hover axial-gust scaling, and local-obstacle residual responses. Runtime blackbox separates adopted A4MC source gust, signed updraft, terrain-shear, and pressure channels, so the packet can now calibrate those channels independently.
- `docs/data/a4mc_disk_gradient_response_packet.csv` adds a `6227`-row A4MC rotor disk-gradient response surface mirrored as `a4mc_disk_gradient_packet_*`. It varies preset, spin state, adopted A4MC disk gradient, and the source-quality availability gate, then records adopted gradient, tip-speed ratio, thrust-loss scale, load, vibration, stall, and flapping proxies from the current core formulas. The runtime blackbox now logs the same disk-gradient thrust-loss/load/vibration/stall response components separately from raw gradient magnitude for trace-to-packet calibration.
- `docs/data/a4mc_local_voxel_coupling_packet.csv` adds a `5049`-row A4MC local-voxel bridge, body-response, and thermal ventilation audit mirrored as `a4mc_local_voxel_packet_*`. It isolates the fabric-side preprocessing that turns trusted local L2 A4MC shelter and pressure samples into source-quality-gated duplicate-obstacle residuals, precipitation exposure attenuation, rotor residual fallback behavior, pressure-gradient disk-wind equivalents, high-contrast pressure saturation ordering, and per-sample quality-blended shelter-gradient side obstruction before the core disk-gradient response, then adds `84` core pressure-center flow scenarios plus `6` motor/ESC/battery ventilation scenarios for adopted asymmetric local-voxel rotor signals. The pressure-center matrix now covers static-unpowered, powered-hover, slow-cruise, and fast-cruise cases, including the small rotor-wash downforce moment used when there is local L2 asymmetry but little freestream airspeed, and it records per-rotor radial pressure-gradient components so tangential disk-gradient pressure cues stay rotor-local instead of moving the whole airframe pressure center. Runtime blackbox and offline CSV output now expose aggregate and per-rotor `local_voxel_obstacle_residual`, precipitation wetness, `rotor_flow_obstruction_wall_force_factor`, `rotor_a4mc_ventilation_efficiency`, per-rotor A4MC ventilation efficiency, and `a4mc_pack_ventilation_efficiency` columns, while the blackbox summary reports the wall-force geometry gate as `wallfac` and the minimum rotor/pack A4MC ventilation efficiency as `a4mcvent`; the offline CLI `Summary:` line reports the same gate as `max_wallfac` and the same minima as `min_a4mc_vent=rotor/pack`, so trace analysis can line up live wall/tunnel attenuation, wet-prop shelter relief, pressure-center moments, and local shelter cooling loss with this packet directly.
...
- In the current runtime wall sampler, `racingQuad` uses a side-flow scan distance of `0.4128 m`, or `6.50R`. At `d/R = 1.0`, where geometric disk overlap is already zero, the current flat-wall obstruction is `0.487`, the wall-force geometry factor is `0.638`, the affected-rotor thrust multiplier is `0.988`, two affected rotors give an equivalent whole-vehicle thrust multiplier of `0.994`, and the wall-attraction force is `0.111 N` per affected rotor / `0.021x` vehicle weight for two rotors.
- The current offline `wall_skim` scene keeps the closest rotor at about `0.04 m` from the wall. That corresponds to `d/R = 0.63` for `racingQuad`, `0.62` for `apDrone`, `1.05` for `cinewhoop`, and `0.31` for `heavyLift`; the A4MC proxy keeps the local-obstacle residual at `0.62x`, mirrors the Fabric bridge's quality-weighted one-sided local-voxel preprocessing with `0.042393` side-shelter obstruction plus `0.0801 m/s` pressure-gradient-equivalent disk wind, runs at body precipitation wetness `0.72` with per-rotor A4MC shelter reducing wet-prop rain exposure, and produces combined closest-rotor obstruction values of `0.393`, `0.396`, `0.342`, and `0.464`. For `racingQuad`, that maps the raw geometric obstruction `0.591` to residual `0.366` plus shelter, with an affected-rotor thrust multiplier of `0.994`.
- The A4MC source-quality packet keeps the synthetic wall-skim source gate explicit: the reference `confidence=0.86`, fresh, `shelter=0.74`, `shear=0.58/block`, `source_turbulence=0.24`, `updraft=0.18 m/s` case adopts source quality `0.86`, source turbulence `0.206`, natural turbulence proxy `0.786`, source-gust Y peak proxy `0.409 m/s`, updraft target/1 s settled values `0.078/0.077 m/s`, a terrain-vector peak proxy of `0.192 m/s`, and local-voxel ventilation efficiency `0.873`. The trusted mixed-ABL rows bracket Dryden response with an unstable intensity proxy of `0.890` and vertical time-scale `0.627x` versus stable intensity `0.499` and vertical time-scale `1.243x`; the full-trust `+/-12 m/s` vertical-flow endpoints clamp to signed `+/-4.5 m/s` targets and settle to `+/-4.499 m/s` after `1 s` at `200 Hz`. The pressure matrix adds `325` quality/freshness/local-vs-coarse scenarios: wall-skim `-12 Pa` produces `-0.1032 hPa` barometer delta and `0.586 m` local static-port error, half confidence gives `0.341 m`, `-220 Pa` saturates at `0.65 m`, coarse flow contributes `0 m`, and stale `+4500 Pa` falls back to `1.0x` density. The mean-overlap sanity rows keep `meanY=3 m/s` plus `updraft=3 m/s` at zero updraft target and zero terrain-vector peak, while `meanY=2 m/s` plus `updraft=5 m/s` leaves a `3 m/s` residual source and terrain signal. A stale `160 tick` source drops quality and transient forcing to zero, so real block-edge traces should keep raw updraft, adopted updraft, terrain-vector peaks, and adopted pressure separate during calibration.
- The A4MC disk-gradient packet keeps a direct-core sensitivity probe separate from Fabric local-voxel preprocessing: once the source-quality gate is available, `0.33 m/s` adopted pressure-gradient wind stays `0.33 m/s` in core response, while quality `0` disables the channel. For `racingQuad` hover, flapping tilt starts near `0.312 m/s` adopted gradient and thrust-loss starts near `0.485 m/s`; the wall-skim probe remains below thrust-loss onset with only `0.00651` load, `0.0109` vibration, and `0.00018 deg` tilt, while a full `12 m/s` gradient at max spin reaches `4.5 deg` tilt and `3.77%` thrust loss in the current formula.
- The A4MC local-voxel coupling packet keeps the fabric bridge separate from the older offline surface handoff: the reference `confidence=0.86`, fresh, `shelter=0.74` case maps duplicate local-obstacle airflow to a `0.419328x` residual and direct precipitation exposure to `0.541792x`, while full trusted shelter clamps exposure to `0.28x` and a stale `160 tick` source returns rain exposure to `1.0x`. Missing/coarse/stale/untrusted rotor-center samples keep the body residual instead of restoring duplicate geometric obstruction. Usable rotor-center samples still refine the residual per rotor, with a trusted exposed rotor at `0.68x` and a trusted fully sheltered rotor clamped to `0.28x`. A one-sided `220 Pa` disk-edge pressure step maps to `0.0801 m/s` quality-weighted disk gradient, and the largest `3200 Pa` pressure step maps to `1.355 m/s`. The saturated `-5000/+5000 Pa` center/edge pressure contrast row shows why runtime applies quality before pressure-to-wind saturation: at `quality=0.5`, quality-first gives `2.118 m/s`, while post-equivalent scaling would give only `1.2 m/s`. For the shelter-gradient wall-skim proxy, center shelter `0.35` plus input edge delta `0.74` is clamped by runtime `WindSample` sanitization to edge shelter `1.0`; the bridge then quality-blends the center sample to `0.301`, fades the wall-side edge sample toward that adopted center to `0.90214`, keeps the unaffected edge samples at `0.34314`, and yields `0.042393` runtime side obstruction. A stale `160 tick` source drops that obstruction to zero.
- The nearfield calibration packet summarizes the tuning warning numerically: for `racingQuad` at `h/R = 1`, current ground multiplier is `1.144` versus ZJU `1.332`, so current extra thrust is `0.433x` the ZJU extra while still `1.072x` the Cheeseman-Bennett boost. At a flat wall `d/R = 0.25`, the current runtime mapping gives two affected rotors an equivalent vehicle thrust loss of `2.05%`, wall-force geometry factor `0.894`, and wall force `0.056x` weight; at `d/R = 1.0`, where ideal disk overlap is zero, it gives `0.58%` vehicle thrust loss, wall-force geometry factor `0.638`, and `0.021x` wall force. The offline A4MC `wall_skim` handoff adds `racingQuad_offline_a4mc_wall_skim_combined_obstruction = 0.393`, `combined_over_geometry = 0.665`, and `pressure_disk_gradient = 0.0801 m/s`, making that scene a synthetic contact/dirty-air case rather than a pure sidewall disk-overlap target. Published sidewall anchors report total lift change below `5%` and negligible total-thrust wall effect, so ordinary sidewall proximity should be modeled primarily as attraction/moment/dirty air rather than a large clean thrust-loss term.
