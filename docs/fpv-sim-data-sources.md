...
- Generated file `docs/data/airframe_cda_guard_packet.csv` builds a narrower CdA and force-law guard from IMAV, NASA, RotorPy, RPG, ICAS, RATM, APdrone, and Manchester anchors; it is mirrored as `airframe_cda_guard_packet_*` categories and records the current `racingQuad` X/Z equivalent `CdA = 0.298/0.301 m^2`, current scale-to-target values of `0.110x` for IMAV 10 m/s, `0.070x` for NASA bare-frame, and `0.234x` for NASA powered full-airframe.
...
- `racingQuad` base drag at 10 m/s is `18.25 N` on body X and `18.45 N` on body Z before separated-flow additions, versus RotorPy Hummingbird's `0.5 N` and `1.0 N` body-drag-only values.
- The IMAV 2022 5-inch three-blade quadrotor fit gives the current `racingQuad` mass a low-speed drag reference of about `0.201 N/(m/s)`, or `2.01 N` at `10 m/s`. The generated airframe-drag table puts current X/Z drag at roughly `9.1x`/`9.2x` that mass-fit reference at `10 m/s`.
- The CdA guard packet makes the coefficient-level issue explicit: at the IMAV 10 m/s target, the current shared `linearDragCoefficient = 0.18 N/(m/s)^2` alone is `8.97x` the total target coefficient, and the target body-drag coefficient would be `-0.15993 N/(m/s)^2` if the shared term were left unchanged.
- Low-precision digitization of NASA Fig. 30 bare-airframe wind-tunnel `Drag/q` gives a small-quad 0 deg median of `0.225 ft^2`, equivalent to `CdA = 0.0209 m^2` and `1.28 N` drag at `10 m/s`. Current `racingQuad` X-axis base drag at 10 m/s is `18.25 N`, about `14.3x` that bare-frame wind-tunnel median. Treat this as a lower-bound body-drag anchor, because it excludes powered rotor/body interaction.
- Low-precision digitization of NASA Figs. 18/20/22 powered full-airframe drag at `q = 0.48 lb/ft^2` gives a mid-RPM small-quad median of `0.36 lbf` / `1.60 N` at about `6.13 m/s`, equivalent to `CdA = 0.0697 m^2`. Current `racingQuad` X-axis base drag at that same dynamic-pressure speed is `6.85 N`, about `4.3x` the powered wind-tunnel median.
...
- UZH-FPV's public SplitS1 sequence lists peak speed `26.79 m/s` (`96.4 km/h`). At that speed, current `racingQuad` X-axis drag is about `131 N`, `2.48x` the preset's level-flight horizontal thrust margin, requiring `2.43x` the configured max total thrust. This turns the drag concern into a flight-envelope feasibility issue, not just a CdA comparison.
- Race Against the Machine adds a second high-speed FPV anchor: 36 open-design racing flights, `500 Hz` synchronized CSVs, `thrust[0-3]`, `vbat`, IMU, RC channels, and motion-capture pose/velocity. The local Range-extraction packet reads all 36 `_500hz_freq_sync.csv` files from the v3.0.0 split release without downloading the full `15.92 GiB` archive: `996104` telemetry rows, `13/36` flights at or above `21 m/s`, fastest sample `21.853 m/s`, and strongest P99 speed `21.259 m/s`. At that fastest sample, current `racingQuad` X-axis drag is `87.16 N`, `1.65x` its level-flight horizontal thrust margin, requiring `1.63x` configured max thrust; the older `21 m/s` README floor alone gives `80.48 N` and `1.50x` max thrust. Its inferred KV is `1.16x` the RATM `2020KV` motor, while per-motor current limit is `0.41x` the RATM `55 A` ESC rating.
- The RATM acceleration/drag residual packet adds a powered-flight kinematic check on top of the speed-envelope rows. Across the six fastest 1 s windows, current drag at each vmax implies median `78.87 m/s^2` drag-only deceleration, while median observed absolute speed-rate at vmax is `5.24 m/s^2`; current drag-decel P95 is `1.38x` the observed-decel P95. This is not a passive coastdown fit, but it is useful evidence that current high-speed drag needs explicit thrust-vector compensation if kept.
...
- The APdrone drag-speed-envelope table makes the bound explicit across `16` selected/open-field GPS speed points and `64` preset/axis constraints. At the `18.72 m/s` fastest point, `racingQuad()` X/Z coefficients `0.1825/0.1845 N/(m/s)^2` exceed the full-thrust level-flight upper coefficient `0.15099 N/(m/s)^2` by `1.21x/1.22x`, leaving `-11.0/-11.7 N` residual horizontal margin. `apDrone()` X/Z coefficients `0.1218/0.1234 N/(m/s)^2` remain below its `0.15309 N/(m/s)^2` upper bound by `0.80x/0.81x`, leaving `11.0/10.4 N` residual margin. Force-equivalent `CdA` values are large because this converts the project's combined damping coefficient directly: `racingQuad()` X/Z is `0.298/0.301 m^2`, `apDrone()` X/Z is `0.199/0.201 m^2`, and the `18.72 m/s` APdrone upper bound is about `0.250 m^2`.
...
- At that same 10 m/s reference point, matching IMAV with a quadratic drag form would require total X-axis coefficient about `0.0201 N/(m/s)^2`, while current `linearDragCoefficient` alone is `0.18`; reducing only `bodyDragCoefficients` cannot reach the physical-drag target unless the shared linear term is also reduced or explicitly treated as gameplay damping.
- Passive coastdown from 20 to 5 m/s makes the gap easier to tune against: the IMAV mass-fit reference takes about `7.60 s` and `82.2 m`, while current `racingQuad` X/Z drag slows in about `0.90 s` and `8.3 m`; initial 20 m/s decel is about `18.2x`/`18.4x` the IMAV fit.
- Against the RPG linear rotor-drag shape, `racingQuad` current X-axis 20 to 5 m/s coastdown time is `0.355x` the RPG-derived time, with `6.10x` the initial 20 m/s deceleration.
- The airframe drag packet makes those warning numbers one-table accessible: current `racingQuad` X drag is `18.25 N` at `10 m/s` and `73.0 N` at `20 m/s`; it is `9.09x` the IMAV 10 m/s mass-fit, `14.25x` the NASA bare-frame 10 m/s median, `4.28x` the NASA powered full-airframe reference-speed median, and `3.05x`/`6.10x` the RPG rotor-drag shape at 10/20 m/s.
...
- RATM thrust-vector takeaway: across `827` samples at or above `21 m/s`, the median body-Z/velocity angle is `77.95 deg`, median mean thrust command is `0.325`, median command-projection proxy is `0.0643`, and median current drag demand is `1.564x` current max thrust. At the six vmax samples, the median command-projection proxy is only `0.0245x` the current drag/max-thrust demand. This is not a calibrated force fit, but it is a strong feasibility guard against retaining the current high-speed drag magnitude without a separate prop-unloading/thrust-vector explanation.
...
- `docs/data/a4mc_source_quality_response_packet.csv` adds a `5996`-row A4MC source-quality response matrix mirrored as `a4mc_source_quality_packet_*`. It varies trust, confidence, freshness, shelter, shear, source turbulence, and a dedicated ABL stability/mixing matrix, then records the core-adopted source turbulence, natural turbulence, ABL Dryden intensity/time-scale, source gust, terrain-shear, local-voxel ventilation, and local-obstacle residual responses. Runtime blackbox now separates adopted A4MC source gust, signed updraft, and terrain-shear air-mass splits, which should be mirrored into the next source-quality packet refresh.
...
- `docs/data/fpv_model_validation_summary.csv` now includes these near-surface rows under `surface_proximity_ground_reference_curve`, `surface_proximity_ground_ceiling_current`, `surface_proximity_wall_force_current`, `surface_jirs2024_packet_*`, and `surface_jirs2024_curve_fit_*`, so automated analysis can use the summary file without reopening the wide source CSV.
...
- JIRS 2024 wall-fit takeaway: terraXcube absolute wall force fits cleanly (`R2=0.923`, predicted `0.1077 N` at `d/R=1`), but the pooled wall-force fit is weak (`R2=0.225`, predicted `0.1655 N` at `d/R=1`). The current `racingQuad` two-affected-rotor wall-force proxy at `d/R=1` is `0.5610 N`, or `3.39x` the pooled JIRS fit; treat that as a possible gameplay/dirty-air scale until a geometry-matched wall experiment supports it.
...
- In the current runtime wall sampler, `racingQuad` uses a side-flow scan distance of `0.3724 m`, or `5.86R`. At `d/R = 1.0`, where geometric disk overlap is already zero, the current flat-wall obstruction is still `0.662`, the affected-rotor thrust multiplier is `0.895`, two affected rotors give an equivalent whole-vehicle thrust multiplier of `0.947`, and the wall-attraction force is `0.281 N` per affected rotor / `0.052x` vehicle weight for two rotors.
- The current offline `wall_skim` scene keeps the closest rotor at about `0.04 m` from the wall. That corresponds to `d/R = 0.63` for `racingQuad`, `0.62` for `apDrone`, `1.05` for `cinewhoop`, and `0.31` for `heavyLift`; the A4MC proxy keeps the local-obstacle residual at `0.62x`, adds `0.150` side-shelter obstruction plus `0.33 m/s` pressure-gradient-equivalent disk wind, and produces combined closest-rotor obstruction values of `0.531`, `0.532`, `0.519`, and `0.549`. For `racingQuad`, that maps the raw geometric obstruction `0.724` to residual `0.449` plus shelter, with an affected-rotor thrust multiplier of `0.932`.
- The A4MC source-quality packet keeps the synthetic wall-skim source gate explicit: the reference `confidence=0.86`, fresh, `shelter=0.74`, `shear=0.58/block`, `source_turbulence=0.24` case adopts source quality `0.86`, source turbulence `0.206`, natural turbulence proxy `0.786`, source-gust Y peak proxy `0.409 m/s`, a terrain-vector peak proxy of `0.192 m/s`, and local-voxel ventilation efficiency `0.873`. The trusted mixed-ABL rows bracket Dryden response with an unstable intensity proxy of `0.890` and vertical time-scale `0.627x` versus stable intensity `0.499` and vertical time-scale `1.243x`; a stale `160 tick` source drops quality and source-gust forcing to zero. Because runtime telemetry now logs adopted A4MC updraft independently from terrain shear, real block-edge traces should keep raw updraft, adopted updraft, and terrain-vector peaks separate during calibration.
...
8. Reconcile drag coefficients.
   ...
   - `racingQuad` produces about `1.69x` weight of base X-axis drag at 10 m/s and about `6.8x` at 20 m/s before separated-flow terms.
   - UZH-FPV speed envelopes show the current high-speed drag force exceeds available level-flight thrust margin at 19-27 m/s; treat coefficients as gameplay damping unless refit from trajectory/force data.
