...
- `docs/data/partial_surface_effect_lead_packet.csv` now maps the partial-surface threshold to current presets: `racingQuad` should treat support patches below about `0.0635 m` diameter as negligible and around `0.127 m` as full-like for one rotor; one full Minecraft block is `7.87D`, so the important missing runtime logic is local edge/overlap area gating, not full-block ground-effect strength.
